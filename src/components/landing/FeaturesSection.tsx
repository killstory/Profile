        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {experiences.map((exp, i) => (
              ...
              >
                {/* Timeline Dot */}
                <div className={`absolute left-4 md:left-12 top-6 -translate-x-1/2 w-4 h-4 rounded-full border-2 ${
                  exp.isPresent 
                    ? 'bg-primary border-primary shadow-[0_0_16px_rgba(var(--primary),0.6)]' 
                ...
                <Link to={`/experience/${exp.slug}`} className="group block">
                  <div className="flex items-center gap-5 md:gap-6 p-5 md:p-6 rounded-2xl border border-transparent hover:border-border hover:bg-muted/30 transition-all duration-300">
                    {/* Thumbnail */}
                    <div className={`relative w-20 h-20 md:w-24 md:h-24 flex-shrink-0 rounded-xl overflow-hidden shadow-md ${
                      exp.logoStyle === 'forru' 
                        ? 'bg-gradient-to-br from-emerald-50 to-green-100 p-3' 
                        : exp.logoStyle === 'mahidol'
                        ? 'bg-gradient-to-br from-slate-50 to-blue-50 p-3'
                        : exp.logoStyle === 'talus'
                        ? 'bg-gradient-to-br from-orange-50 to-amber-100 p-3'
                        : ''
                    }`}>
                      <img
                        src={exp.image}
